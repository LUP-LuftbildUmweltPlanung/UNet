...
def load_fastai_model_flexible(model_uri):
    ...
    if model_uri.startswith("mlflow-artifacts:/") or model_uri.startswith("runs:/"):
        ...
        return load_learner(model_uri, cpu=False)
...
def predict_with_tta(learn, tile_path):
    ...
    with rasterio.open(tile_path) as src:
        image = src.read([1, 2, 3, 4, 5]).transpose(1, 2, 0)  # shape (H, W, 5)
    ...
    for name, t in transforms:
        ...
        tensor = torch.from_numpy(img_aug.astype(np.float32) / 65535.0) \
